...
from free_proxy import free_proxy

# Disable only the InsecureRequestWarning
urllib3.disable_warnings(urllib3.exceptions.InsecureRequestWarning)

# MongoDB setup
MONGO_URI = os.environ.get("mongodb+srv://comeprimasgtsz_db_user:7V1l8DuuKMBTApJz@teste.wyqsfmt.mongodb.net/?retryWrites=true&w=majority&appName=teste", "mongodb://localhost:27017/")
...
def get_fresh_proxy():
    try:
        print("[INFO] Buscando novos proxies...")
        proxies = free_proxy.get(anonymity='high', country_id=['BR'], https=True, timeout=5, limit=10)
        if not proxies:
            print("[WARN] Nenhum proxy brasileiro de alta anonimidade encontrado. Tentando sem filtro de país.")
            proxies = free_proxy.get(anonymity='high', https=True, timeout=5, limit=10)
        if not proxies:
            print("[WARN] Nenhum proxy de alta anonimidade encontrado. Tentando com qualquer anonimidade.")
            proxies = free_proxy.get(https=True, timeout=5, limit=10)
        if proxies:
            # free_proxy returns a list of dictionaries, each with 'ip' and 'port'
            # We need to format it as 'http://ip:port' or 'https://ip:port'
            formatted_proxies = [f"http://{p['ip']}:{p['port']}" for p in proxies]
            print(f"[INFO] Proxies encontrados: {formatted_proxies}")
            return formatted_proxies
        else:
            print("[ERROR] Nenhum proxy funcional encontrado após várias tentativas.")
            return []
    except Exception as e:
        print(f"[ERROR] Erro ao buscar proxies: {e}")
        return []
